...
export async function sendLimitReachedMarketingEmail(
  ...
  try {
    ...
    const subject = isHi
      ? `⏰ ${featureLabel} की लिमिट पूरी — अभी अपग्रेड करें (30% OFF)`
      : `⏰ ${featureLabel} limit reached — Upgrade now & save 30%`;

    const headline = isHi
      ? 'आपकी लिमिट खत्म हो चुकी है'
      : 'You\'ve hit your plan limit';

    const sub = isHi
      ? `${featureLabel} पर आज और इस्तेमाल नहीं हो सकता। प्रो पर अपग्रेड करें और बिना रुकावट चलाते रहें।`
      : `You can't use ${featureLabel} again until your limit resets. Upgrade to Pro to keep going — without interruption.`;

    const bullets = isHi
      ? [
          { e: '🚀', t: '10,000+ क्रिएटर्स पहले से ही प्रो पर हैं — आप अकेले नहीं' },
          { e: '⚡', t: 'अनलिमिटेड AI टूल्स — Daily Ideas, Script, Thumbnail, Shorts, सब' },
          { e: '📈', t: 'औसत 11.8% CTR बूस्ट और 500M+ views जनरेट हो चुके' },
          { e: '🔥', t: 'सीमित समय: इस हफ्ते 30% छूट — कोड अपने आप लागू' },
        ]
      : [
          { e: '🚀', t: 'Join 10,000+ creators already on Pro — you\'re not alone' },
          { e: '⚡', t: 'Unlimited AI tools — Daily Ideas, Script Writer, Thumbnail, Shorts, all of it' },
          { e: '📈', t: '11.8% average CTR boost & 500M+ views generated by Pro users' },
          { e: '🔥', t: 'Limited time: 30% off this week — discount auto-applied' },
        ];

    const testimonial = isHi
      ? '"प्रो पर अपग्रेड करने के 8 महीने में 30K से 210K subscribers — AI script writer अकेला Pro plan worth करता है।" — रोहित एस., टेक क्रिएटर'
    ...
    const ps = isHi
      ? 'P.S. अभी अपग्रेड करने पर discount तुरंत लागू हो जाएगा। डिस्काउंट जल्दी ही खत्म हो रहा है।'
      : 'P.S. The 30% discount applies instantly when you upgrade now. Offer ends soon.';

    const bulletsHtml = bullets
      .map(
        (b) =>
          `<tr><td style="padding:8px 0;font-size:15px;line-height:1.5;color:#1a1a1a;"><span style="display:inline-block;width:28px;font-size:18px;">${b.e}</span>${b.t}</td></tr>`,
      )
      .join('');

    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="margin:0;padding:0;background:#f4f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;"><div style="max-width:600px;margin:0 auto;background:#ffffff;"><div style="background:linear-gradient(135deg,#FF0000 0%,#CC0000 100%);color:white;padding:32px 24px;text-align:center;">${getEmailLogoHtml()}<div style="font-size:32px;margin-top:16px;">⏰</div><h1 style="margin:8px 0 0;font-size:26px;font-weight:800;">${headline}</h1></div><div style="padding:32px 28px;"><p style="font-size:16px;color:#1a1a1a;margin:0 0 12px;">Hello ${userName || (isHi ? 'क्रिएटर' : 'Creator')},</p><p style="font-size:16px;color:#444;line-height:1.55;margin:0 0 24px;">${sub}</p><div style="background:#fef2f2;border-left:4px solid #FF0000;padding:18px 20px;border-radius:6px;margin:0 0 24px;"><table style="width:100%;border-collapse:collapse;">${bulletsHtml}</table></div><div style="text-align:center;margin:28px 0;"><a href="${upgradeUrl}" style="display:inline-block;background:#FF0000;color:#fff;text-decoration:none;font-weight:700;font-size:17px;padding:16px 36px;border-radius:8px;box-shadow:0 4px 14px rgba(255,0,0,0.35);">${cta} →</a></div><blockquote style="border-left:3px solid #ddd;margin:24px 0;padding:8px 16px;color:#555;font-style:italic;font-size:14px;line-height:1.5;">${testimonial}</blockquote><p style="font-size:13px;color:#666;text-align:center;margin:24px 0 8px;">${guarantee}</p><p style="font-size:14px;color:#888;margin:24px 0 0;font-style:italic;">${ps}</p></div><div style="background:#0F0F0F;color:#aaa;padding:20px;text-align:center;font-size:12px;"><p style="margin:0;">© ${new Date().getFullYear()} Vid YT · <a href="${getEmailBaseUrl()}/subscription" style="color:#FF6666;">Manage Plan</a></p></div></div></body></html>`;
